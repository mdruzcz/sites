...
const INTERNAL = /(\/(?:contact|services\/[a-z0-9-]+\/[a-z0-9-]+|services\/[a-z0-9-]+|services|finishes\/[a-z0-9-]+|finishes|warranty|service-areas\/[a-z0-9-]+|service-areas|gallery|faq|about|resources\/[a-z0-9-]+|resources)\b)/g;

function linkify(text: string): ReactNode[] {
  return text.split(INTERNAL).map((part, i) =>
    /^\/[a-z]/.test(part) ? <Link key={i} href={part} className="font-semibold text-[var(--accent-deep)] underline decoration-[var(--accent-soft)] decoration-2 underline-offset-2 hover:decoration-[var(--accent)]">{part}</Link> : <span key={i}>{part}</span>
  );
}
