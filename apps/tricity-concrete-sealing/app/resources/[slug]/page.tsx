...
export default async function GuidePage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return (
    <>
      ...
      <article className="bg-[var(--stone)]">
                ...
                <div className="mt-6 space-y-3">
                  {a.faq.map((f, i) => (
                    <details key={i} className="group card">
                      <summary className="flex min-h-[56px] cursor-pointer list-none items-center justify-between gap-4 px-5 py-4 font-bold">{f.q}<span aria-hidden className="grid size-7 shrink-0 place-items-center rounded-full bg-[var(--stone)] text-[var(--accent)] transition group-open:rotate-45">+</span></summary>
