...
export async function POST(req: Request) {
  ...
  const verifyRes = await fetch(
    process.env.TURNSTILE_VERIFY_ENDPOINT ?? "https://turnstile.masterdecker.com",
    {
      ...
      body: JSON.stringify({ token: body.token, hostname: "torontodeckstainers.ca" }),
    },
  );
  ...
  const insertRes = await fetch(formsEndpoint, {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify({ hostname: "torontodeckstainers.ca", row: payload }),
  });
  if (!insertRes.ok) {
  ...
  if (process.env.RESEND_API_KEY) {
    await fetch("https://api.resend.com/emails", {
      ...
      body: JSON.stringify({
        from: process.env.CONTACT_FROM_EMAIL || "noreply@torontodeckstainers.ca",
