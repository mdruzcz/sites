...
import { articles, getArticle, getRelatedArticles, articlePhoto } from "@/lib/resources";
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const img = photo(articlePhoto(article)).src;
  return {
    title: article.metaTitle.replace(/\s*\|\s*Permanent Lighting Direct\s*$/i, ""),
...
export default async function GuidePage({ params }: Props) {
  ...
  const heroKey = articlePhoto(article);
  ...
  const jsonLd = [
    {
      ...
      author: { "@type": "Organization", name: "Permanent Lighting Direct", url: SITE_URL },
      publisher: { "@type": "Organization", name: "Permanent Lighting Direct", url: SITE_URL }
    },
    {
      "@context": "https://schema.org",
      "@type": "FAQPage",
        ...
        <div className="shell section">
          <div className="mx-auto max-w-[72ch]">
            ...
            <div className="card mt-10 p-7">
              <p className="eyebrow text-[var(--color-accent-dark)]">Key takeaways</p>
          ...
          {related.length > 0 && (
            <section className="mx-auto mt-20 max-w-6xl">
              <p className="eyebrow eyebrow-rule text-[var(--color-accent-dark)]">Keep reading</p>
              <div className="mt-8 grid gap-5 sm:grid-cols-3">
                {related.map((r) => (
                  <Link key={r.slug} href={`/resources/${r.slug}`} className="card card-lift group flex flex-col p-6">
                    ...
                    <h3 className="font-display mt-2 text-lg leading-snug transition group-hover:text-[var(--color-accent-dark)]">{r.title}</h3>
