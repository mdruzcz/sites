...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  return {
    title: (product.meta_title ?? product.name).replace(/\s*\|\s*Permanent Lighting Direct\s*$/i, ""),
  ...
  const jsonLd = [
    {
      ...
      brand: { "@type": "Brand", name: "Permanent Lighting Direct" },
  ...
  return (
    <div className="shell py-8 md:py-12">
      ...
      <div className="mt-5 grid gap-10 lg:grid-cols-[1.05fr_1fr]">
        ...
        <div>
          <p className="eyebrow text-[var(--color-accent-dark)]">{category?.name ?? "Permanent Lighting Direct"}</p>
