...
export async function generateMetadata({
  ...
  return {
    title: `${post.title.replace(/&amp;/g, "&")} | Woodstock Deck & Fence`,
...
function renderBlock(block: PostBlock, i: number) {
  switch (block.type) {
    case "h2":
      return (
        <h2 key={i} className="font-serif text-2xl lg:text-3xl font-bold mt-10 mb-4" style={{ color: "var(--forest)" }}>
          {block.text}
        </h2>
      );
    case "h3":
      return (
        <h3 key={i} className="font-serif text-xl font-bold mt-8 mb-3" style={{ color: "var(--forest)" }}>
          {block.text}
        </h3>
      );
    case "p":
    ...
    case "callout":
      return (
        <aside
          ...
          style={{ backgroundColor: "var(--cream-dark)", borderColor: "var(--cedar)" }}
        >
          <h4 className="font-serif text-lg font-bold mb-2" style={{ color: "var(--forest)" }}>
            {block.title}
          </h4>
...
export default async function BlogPostPage({
  ...
  const articleSchema = {
    ...
    author: { "@type": "Organization", name: "Woodstock Deck & Fence" },
    publisher: {
      "@type": "Organization",
  ...
  return (
    <main>
      ...
      <article>
        <header className="pt-32 pb-8" style={{ backgroundColor: "var(--forest)" }}>
          <div className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8">
            <Link href="/blog" className="text-sm" style={{ color: "var(--cedar-light)" }}>
              ← All guides
            </Link>
            <div className="flex items-center gap-3 mt-4 mb-4">
              <span className="text-xs font-semibold uppercase tracking-widest px-2.5 py-1 rounded-full" style={{ backgroundColor: "rgba(196,98,58,0.25)", color: "var(--cedar-light)" }}>
                {post.category}
              </span>
      ...
      <section className="py-16 lg:py-20" style={{ backgroundColor: "var(--cream-dark)" }}>
        <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
          <h2 className="font-serif text-2xl lg:text-3xl font-bold mb-8 text-center" style={{ color: "var(--forest)" }}>
            Keep reading
          </h2>
          <div className="grid md:grid-cols-3 gap-6">
            {related.map((p) => (
              <Link key={p.slug} href={`/blog/${p.slug}`} className="block bg-white rounded-2xl border overflow-hidden hover:shadow-lg transition" style={{ borderColor: "var(--cream)" }}>
                ...
                <div className="p-5">
                  <p className="text-xs font-semibold uppercase tracking-widest mb-2" style={{ color: "var(--cedar)" }}>
                    {p.category}
                  </p>
                  <h3 className="font-serif text-lg font-bold leading-tight" style={{ color: "var(--forest)" }} dangerouslySetInnerHTML={{ __html: p.title }} />
                </div>
              </Link>
            ))}
          </div>
