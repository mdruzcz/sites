...
export default async function sitemap(): Promise<MetadataRoute.Sitemap> {
  ...
  return [
    ...pages.map(([p, priority, changeFrequency]) => ({ url: `${SITE_URL}${p}`, lastModified: now, priority, changeFrequency })),
    ...INSTALL_CITIES.map((c) => ({ url: `${SITE_URL}/installation/${c.slug}`, lastModified: now, priority: 0.75, changeFrequency: "monthly" as const })),
