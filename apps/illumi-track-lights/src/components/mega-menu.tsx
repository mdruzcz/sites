...
export const SHOP_COLUMNS: MenuColumn[] = [
  {
    ...
    links: [
      ...
      { label: "250 ft kit", href: "/product/led-housing-package-250" },
  ...
  },
  {
    ...
    links: [
      ...
      { label: "12V soffit downlight kit", href: "/product/12v-led-soffit-light-kit" },
  ...
  },
  {
    ...
    links: [
      ...
      { label: "40-pack white track", href: "/product/40-white-track" },
...
export function MegaMenu() {
  ...
  return (
    <div onMouseEnter={openNow} onMouseLeave={closeSoon} className="static">
      <button type="button" onClick={() => setOpen((o) => !o)} aria-expanded={open} aria-haspopup="true" className="nav-link inline-flex items-center gap-1.5">
        Shop
      ...
      {open && (
        <div onMouseEnter={openNow} onMouseLeave={closeSoon} className="absolute inset-x-0 top-full z-40 border-t border-white/10 bg-[var(--color-ink)] text-white shadow-[var(--shadow-xl)]">
          <div className="shell grid gap-8 py-8 lg:grid-cols-[1.15fr_1fr_1fr_1fr_1.1fr]">
            {SHOP_COLUMNS.map((col) => (
              <div key={col.title}>
                <Link href={col.href} onClick={() => setOpen(false)} className="eyebrow text-[var(--color-gold)] hover:text-white">{col.title}</Link>
                <ul className="mt-4 space-y-2.5">
                  {col.links.map((l) => (
                    <li key={l.href + l.label}>
                      <Link href={l.href} onClick={() => setOpen(false)} className="text-sm text-white/80 transition hover:text-white">{l.label}</Link>
                    </li>
                  ))}
                </ul>
              </div>
            ...
            <Link href="/installation" onClick={() => setOpen(false)} className="group relative hidden overflow-hidden rounded-2xl lg:block">
              ...
              <span className="absolute inset-x-0 bottom-0 p-5">
                ...
                <span className="font-display mt-1 block text-lg">We install it for you</span>
                <span className="mt-1 block text-xs text-white/75">Free on-site measurement, same hardware, 5-year warranty.</span>
              </span>
            </Link>
          </div>
        </div>
