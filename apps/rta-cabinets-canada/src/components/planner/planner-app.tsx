...
import { PlannerContext, type PlannerUI, type Step, type StockInfo } from "./planner-context";
...
export default function PlannerApp({ initialDesign }: { initialDesign?: Design }) {
  ...
  // Live inventory (server reads Supabase; 60s cache)
  useEffect(() => {
    let cancelled = false;
    fetch("/api/inventory")
      .then((r) => (r.ok ? r.json() : {}))
      .then((data: Record<string, StockInfo>) => {
        if (!cancelled && data && typeof data === "object") setStock(data);
      })
      .catch(() => {});
    return () => {
      cancelled = true;
  ...
  return (
    <PlannerContext.Provider value={ctx}>
      <div className="planner-shell no-print">
            ...
            <nav aria-label="Planner steps" className="flex flex-1 items-stretch gap-1 overflow-x-auto scrollbar-thin">
              {STEPS.map((s) => (
                <button
                  ...
                  className={`group flex shrink-0 items-center gap-2 border-b-2 px-2 py-1.5 text-left ${ui.step === s.n ? "border-[var(--color-accent)]" : "border-transparent hover:border-[var(--color-border)]"}`}
                >
                  <span className={`flex h-6 w-6 items-center justify-center rounded-full text-[11px] font-semibold ${ui.step === s.n ? "bg-[var(--color-ink)] text-white" : ui.step > s.n ? "bg-[var(--color-accent)] text-[var(--color-ink)]" : "bg-[var(--color-cream)] text-[var(--color-ink-soft)]"}`}>
                    {ui.step > s.n ? "✓" : s.n}
                  </span>
                  <span>
                    <span className={`block text-[13px] font-medium leading-tight ${ui.step === s.n ? "text-[var(--color-ink)]" : "text-[var(--color-ink-soft)]"}`}>{s.label}</span>
            ...
            <div className="flex items-center gap-2 border-l border-[var(--color-border)] pl-3">
              {editingName ? (
                <input
                  ...
                  className="h-9 w-40 rounded-md border border-[var(--color-border)] px-2 text-sm"
                  aria-label="Design name"
                />
              ) : (
                <button type="button" onClick={() => setEditingName(true)} className="max-w-[160px] truncate text-left text-[13px] font-medium text-[var(--color-ink)] underline-offset-4 hover:underline" title="Rename design">
                  {design.name}
                </button>
              )}
        ...
        <div className="relative">
          ...
          {ui.panel !== "none" && (
            <>
              ...
              <div className="fixed inset-y-0 right-0 z-50 w-full max-w-[380px] border-l border-[var(--color-border)] bg-[var(--color-cream)] shadow-2xl">
        ...
        {toastMsg && (
          <div role="status" className="pointer-events-none fixed bottom-36 left-1/2 z-[60] w-[calc(100%-2rem)] max-w-md -translate-x-1/2 rounded-md bg-[var(--color-ink)] px-4 py-2 text-center text-sm text-white shadow-lg lg:bottom-8 lg:w-auto">
            {toastMsg}
          </div>
        )}
      </div>
