...
export function WallView({ design, surface, selectedId, hoverId, readonly, showDims = true, onSelect, onHover, onDragStart, onMove, onDropSku, className }: WallViewProps) {
  ...
  return (
    ...
    >
      ...
      {surface !== "island" && (
        <>
          <line x1={0} y1={0} x2={0} y2={H} stroke={openLeft ? "#d6cfc4" : NAVY} strokeWidth={openLeft ? 0.6 : 2} strokeDasharray={openLeft ? "3 2" : undefined} />
          <line x1={L} y1={0} x2={L} y2={H} stroke={openRight ? "#d6cfc4" : NAVY} strokeWidth={openRight ? 0.6 : 2} strokeDasharray={openRight ? "3 2" : undefined} />
        </>
      )}
      ...
      {openings.map((o) => {
        ...
        return o.kind === "window" ? (
          ...
          <g key={o.id}>
            ...
            <rect x={o.t + 3} y={y + 3} width={o.width - 6} height={o.height - 3} fill="none" stroke="#d6cfc4" strokeWidth={0.5} />
