...
export async function POST(req: Request) {
  ...
  const verifyRes = await fetch(
    process.env.TURNSTILE_VERIFY_ENDPOINT ?? "https://turnstile.masterdecker.com",
    {
      ...
      body: JSON.stringify({ token: body.token, hostname: "allsealconcretesealing.ca" }),
    },
  );
  ...
  await fetch(
    `${process.env.NEXT_PUBLIC_SUPABASE_URL}/rest/v1/allseal_quote_requests`,
    {
      method: "POST",
      headers: {
  ...
  if (process.env.RESEND_API_KEY) {
    await fetch("https://api.resend.com/emails", {
      ...
      body: JSON.stringify({
        from: process.env.CONTACT_FROM_EMAIL || "noreply@allsealconcretesealing.ca",
        to: process.env.CONTACT_TO_EMAIL || "service@masterdecker.com",
        subject: `New Quote Request: ${body.service} — ${body.name}`,
        html: `
          <h2>New Quote Request — All-Seal Concrete Sealing</h2>
