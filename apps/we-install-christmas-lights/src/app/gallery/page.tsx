...
import library from "@/content/photos.json";
...
// Curated gallery photos plus the wider photo library (same shoot, more angles), de-duplicated by filename.
const seen = new Set<string>();
const photos: GalleryItem[] = [
  ...(gallery as GalleryItem[]),
  ...(library as { src: string; alt: string; category: string; width: number; height: number; blurDataURL: string }[]).map((p) => ({ image: p.src, alt: p.alt, category: p.category, width: p.width, height: p.height, blurDataURL: p.blurDataURL })),
].filter((p) => { const k = p.image.split("/").pop()!; if (seen.has(k)) return false; seen.add(k); return true; });
