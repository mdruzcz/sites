...
export async function POST(req: Request) {
  ...
  const verifyRes = await fetch(
    process.env.TURNSTILE_VERIFY_ENDPOINT ?? "https://turnstile.masterdecker.com",
    {
      ...
      body: JSON.stringify({ token: body.token, hostname: "concretetilsonburg.ca" }),
    },
  );
  ...
  if (process.env.NEXT_PUBLIC_SUPABASE_URL && process.env.NEXT_PUBLIC_SUPABASE_ANON_KEY) {
    await fetch(
      `${process.env.NEXT_PUBLIC_SUPABASE_URL}/rest/v1/concrete_tilsonburg_quote_requests`,
      {
        method: "POST",
        headers: {
  ...
  if (process.env.RESEND_API_KEY) {
    await fetch("https://api.resend.com/emails", {
      ...
      body: JSON.stringify({
        from: process.env.CONTACT_FROM_EMAIL || "noreply@concretetilsonburg.ca",
        to: process.env.CONTACT_TO_EMAIL || "service@masterdecker.com",
        subject: `New Estimate Request: ${body.service} — ${body.name}`,
        html: `
          <h2>New Estimate Request — Concrete Tilsonburg</h2>
