...
export function QuoteForm({ defaultService, variant = "card" }: Props) {
    ...
    try {
      ...
      if (res.ok) {
        ...
        if (typeof window !== "undefined") {
          // @ts-expect-error - umami is added at runtime
  ...
  if (status === "sent") {
    return (
      <div id="quote-success" className={variant === "card" ? "card p-8 text-center" : "p-8 text-center"}>
        ...
        <h3 className="text-xl font-bold text-[var(--charcoal)] mb-2">Quote Request Sent!</h3>
  ...
  return (
    <form onSubmit={handleSubmit} className={`${variant === "card" ? "card" : ""} p-6 sm:p-8 space-y-4`}>
      <div>
        <h3 className="text-xl font-bold mb-1 text-[var(--charcoal)]">Get a Free Quote</h3>
        <p className="text-sm text-[var(--concrete)]">No-obligation. We reply within {site.responseTime}.</p>
      </div>
      ...
      <div>
        <label htmlFor="address" className="block text-sm font-medium text-slate-700 mb-1">Project Address</label>
        <input
          id="address"
          name="address"
          type="text"
      ...
      <div>
        ...
        <textarea
          ...
          name="message"
          rows={3}
          placeholder="Approximate size, timing, photos welcome..."
