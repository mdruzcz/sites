...
export async function generateMetadata({
  ...
  const description = `Professional ${service.title.toLowerCase()} in ${city.name}, Ontario. Reinforced, written warranty, free on-site quote. Call ${site.phone}.`;
...
export default async function ServiceCityPage({
  ...
  const otherServices = getServices().filter((s) => s.slug !== service.slug).slice(0, 4);
      ...
      <section className="bg-charcoal text-white relative overflow-hidden">
        ...
        <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16 sm:py-20">
          ...
          <p className="text-lg text-[var(--concrete-200)] max-w-3xl">
            Local crews, written warranty, free on-site quote. Professional {service.title.toLowerCase()} for {city.name} homeowners — pour after pour.
          </p>
        </div>
      </section>
            ...
            <div className="lg:col-span-2 prose-cd">
              ...
              <ul className="not-prose space-y-3">
                ...
                <li className="flex items-start gap-3">
                  ...
                  <span className="text-[var(--charcoal)]">Local crews who know {city.name} soil and weather patterns</span>
                </li>
              </ul>
              ...
              <div className="mt-10 p-6 bg-[var(--surface)]/50 rounded-xl border-l-4 border-[var(--accent)]">
                <h3 className="font-bold text-lg text-[var(--charcoal)] mb-2">Ready to start your {city.name} project?</h3>
                <p className="text-[var(--concrete)] mb-4">
                  Call now for a free on-site quote on {service.title.toLowerCase()} in {city.name}, ON. We respond within {site.responseTime}.
                </p>
