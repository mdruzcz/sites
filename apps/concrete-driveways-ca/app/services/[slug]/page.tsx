...
export async function generateMetadata({
  ...
  if (!service) return {};
  const title = `${service.title} in London, ON & Area`;
  const description = `${service.shortDescription} Free on-site quote. Call ${site.phone}.`;
  return {
    title,
    description,
    openGraph: {
  ...
  return (
    <>
      ...
      <section className="bg-charcoal text-white relative overflow-hidden">
        <Image
          src={service.image}
          alt={`${service.title} by Concrete Driveways in London, ON`}
          fill
          priority
            ...
            <div className="lg:col-span-2">
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    {projects.slice(0, 4).map((p) => (
                      <div key={p.slug} className="relative aspect-[4/3] rounded-xl overflow-hidden group">
                        ...
                        />
                        <div className="absolute bottom-0 left-0 right-0 bg-gradient-to-t from-[var(--charcoal)]/85 to-transparent p-3">
              ...
              <div className="mt-12 p-6 bg-[var(--surface)]/40 rounded-xl border-l-4 border-[var(--accent)]">
                <h3 className="font-bold text-lg text-[var(--charcoal)] mb-2">
                  {service.title} in Southwestern Ontario
                </h3>
                <p className="text-[var(--concrete)] mb-4">
                  We pour {service.title.toLowerCase()} across the following communities:
                </p>
