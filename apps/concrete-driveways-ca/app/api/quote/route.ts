...
export async function POST(req: Request) {
  ...
  const verifyRes = await fetch(
    process.env.TURNSTILE_VERIFY_ENDPOINT ?? "https://turnstile.masterdecker.com",
    {
      ...
      body: JSON.stringify({ token: body.token, hostname: "concretedriveways.ca" }),
    },
  );
  ...
  if (process.env.RESEND_API_KEY) {
    await fetch("https://api.resend.com/emails", {
      ...
      body: JSON.stringify({
        from: process.env.CONTACT_FROM_EMAIL || "noreply@concretedriveways.ca",
        to: process.env.CONTACT_TO_EMAIL || "service@masterdecker.com",
        subject: `New Quote Request: ${body.service} — ${body.name}`,
        html: `
          <h2>New Quote Request — Concrete Driveways</h2>
