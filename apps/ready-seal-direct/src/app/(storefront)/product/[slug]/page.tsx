...
export default async function ProductPage({ params }: PageProps) {
  ...
  // Brush upsell — every stain order should be offered the applicator brush
  const isBrush = product.slug === "synthetic-stain-brush";
  const brush = isBrush ? null : await getProduct("synthetic-stain-brush");
  const brushVariant = brush?.ecom_variants?.find((v) => v.is_active);
  const brushImg = brush ? primaryImage(brush) : null;
  const brushUpsell =
    brushVariant && brush
      ? {
          variantId: brushVariant.id,
          name: brush.name,
          price: Number(brushVariant.price_cad),
          image: brushImg?.public_url ?? null
        }
      : undefined;
  ...
  const jsonLd = {
    ...
    brand: { "@type": "Brand", name: "Ready Seal" },
    offers: {
      "@type": "AggregateOffer",
      priceCurrency: "CAD",
      lowPrice: min,
  ...
  return (
    <div className="mx-auto max-w-6xl px-4 py-10">
      ...
      <div className="mt-4 grid gap-10 lg:grid-cols-2">
        ...
        <div>
          <p className="eyebrow text-[var(--color-brand)]">Ready Seal · Stain &amp; Sealer</p>
          ...
          <ul className="mt-6 grid grid-cols-2 gap-2 text-xs text-slate-600">
            <li className="flex items-center gap-2 rounded-md bg-white border border-[var(--color-border)] px-3 py-2">
              🚚 Free over $750 (Ontario)
            </li>
            <li className="flex items-center gap-2 rounded-md bg-white border border-[var(--color-border)] px-3 py-2">
              ↩️ 30-day returns
            </li>
            <li className="flex items-center gap-2 rounded-md bg-white border border-[var(--color-border)] px-3 py-2">
              ☀️ Oil-based · UV protection
            </li>
            <li className="flex items-center gap-2 rounded-md bg-white border border-[var(--color-border)] px-3 py-2">
              🍁 Ships from Belmont, ON
            </li>
          </ul>
