    ...
    <header className="sticky top-0 z-50 bg-white shadow-sm">
      ...
      <nav className="container mx-auto px-4 flex items-center justify-between h-16">
        <Link href="/" className="flex items-center gap-2 flex-shrink-0">
          <Image src="/images/logo.png" alt="London Retaining Walls logo" width={160} height={50} className="h-10 w-auto object-contain" priority />
        </Link>
          ...
          >
            ...
            {servicesOpen && (
              <div className="absolute top-full left-0 bg-white shadow-xl rounded-xl border border-gray-100 p-4 w-64 z-50">
                ...
                {site.services.map((s) => (
                  <Link key={s.href} href={s.href} className="block px-2 py-1.5 rounded-lg hover:bg-amber-50 hover:text-[var(--accent)] transition-colors text-sm">
                    {s.name}
                  </Link>
                ))}
                <hr className="my-2 border-gray-100" />
                <Link href="/services" className="block px-2 py-1.5 rounded-lg text-[var(--accent)] font-semibold text-sm hover:bg-amber-50 transition-colors">
                  View All Services →
                </Link>
              </div>
            )}
      ...
      {menuOpen && (
        <div className="lg:hidden bg-white border-t border-gray-100 px-4 py-4 space-y-1">
          {[
            ...
            <Link key={item.href} href={item.href} className="block py-2 px-3 rounded-lg hover:bg-amber-50 hover:text-[var(--accent)] font-medium" onClick={() => setMenuOpen(false)}>
              {item.label}
            </Link>
          ))}
