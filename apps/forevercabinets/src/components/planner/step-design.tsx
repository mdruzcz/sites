...
export function StepDesign() {
  ...
  return (
    <div className="planner-design lg:grid lg:h-[calc(100dvh-192px)] lg:min-h-[600px] lg:grid-cols-[300px_minmax(0,1fr)_320px]">
      {/* Catalog */}
      <aside className={`border-r border-[var(--color-line)] bg-[var(--color-cream)] lg:block ${mobileTab === "catalog" ? "block" : "hidden"} h-[calc(100dvh-230px)] lg:h-auto lg:min-h-0`}>
      ...
      <section className={`flex min-h-0 flex-col bg-[#f7f4ee] ${mobileTab === "canvas" ? "flex" : "hidden lg:flex"}`}>
        <div className="flex flex-wrap items-center gap-2 border-b border-[var(--color-line)] bg-white px-3 py-2">
          <div className="inline-flex border border-[var(--color-line)]" role="tablist" aria-label="View">
            {viewTabs.map((v) => (
              <button
                key={v.id}
                type="button"
                ...
                className={`px-3 py-1.5 text-[11px] uppercase tracking-widest ${ui.view === v.id ? "bg-[var(--color-navy)] text-white" : "bg-white text-[var(--color-ink-soft)] hover:text-[var(--color-navy)]"}`}
              >
                {v.label}
              </button>
            ))}
            ...
            <div className="flex flex-wrap items-center gap-1">
              {([0, 1, 2, 3] as WallId[]).map((w) => (
                <button
                  ...
                  className={`rounded-full border px-2.5 py-1 text-[11px] ${ui.activeSurface === w ? "border-[var(--color-navy)] bg-[var(--color-navy)] text-white" : "border-[var(--color-line)] bg-white disabled:opacity-40"}`}
                >
                  {WALL_NAMES[w]}
                </button>
              ))}
              {design.island.enabled && (
                <button type="button" onClick={() => setUi({ activeSurface: "island" })} className={`rounded-full border px-2.5 py-1 text-[11px] ${ui.activeSurface === "island" ? "border-[var(--color-navy)] bg-[var(--color-navy)] text-white" : "border-[var(--color-line)] bg-white"}`}>
                  Island
                </button>
              )}
            </div>
          ...
          {ui.view === "3d" && (
            <div className="flex flex-wrap items-center gap-1">
              {(["iso", "top", "front", "left", "right"] as CameraPreset[]).map((p) => (
                <button key={p} type="button" onClick={() => sceneApi.current?.setPreset(p)} className="rounded-full border border-[var(--color-line)] bg-white px-2.5 py-1 text-[11px] capitalize hover:border-[var(--color-navy)]">
                  {p === "iso" ? "Reset view" : p}
                </button>
              ))}
            </div>
        ...
        <div className="flex items-center justify-between gap-2 border-t border-[var(--color-line)] bg-white px-3 py-2">
      ...
      {/* Inspector */}
      <aside className={`border-l border-[var(--color-line)] bg-[var(--color-cream)] lg:block ${mobileTab === "inspector" ? "block" : "hidden"} h-[calc(100dvh-230px)] lg:h-auto lg:min-h-0`}>
        <Inspector />
      </aside>

      {/* Mobile tab bar */}
      <nav className="sticky bottom-0 z-30 grid grid-cols-3 border-t border-[var(--color-line)] bg-white lg:hidden" aria-label="Planner panels">
        {(
          [
            ["catalog", "Cabinets"],
            ["canvas", "Plan"],
          ...
          <button key={id} type="button" onClick={() => setMobileTab(id)} className={`h-12 text-[11px] uppercase tracking-widest ${mobileTab === id ? "bg-[var(--color-navy)] text-white" : "text-[var(--color-ink-soft)]"}`}>
            {label}
          </button>
        ))}
      </nav>
