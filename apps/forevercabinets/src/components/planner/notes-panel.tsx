  ...
  return (
    <div className="flex h-full min-h-0 flex-col">
      <header className="flex items-center justify-between border-b border-[var(--color-line)] px-4 py-3">
        <div>
          <p className="text-[10px] uppercase tracking-[0.25em] text-[var(--color-brass-dark)]">Design notes</p>
          <h3 className="font-display text-lg text-[var(--color-navy)]">Notes &amp; reminders</h3>
        </div>
      ...
      <div className="border-b border-[var(--color-line)] p-4">
        ...
        <textarea
          ...
          className="mt-1 block w-full rounded-sm border border-[var(--color-line)] bg-white p-2 text-sm focus:border-[var(--color-navy)] focus:outline-none"
          onKeyDown={(e) => {
      ...
      <ul className="min-h-0 flex-1 overflow-y-auto scrollbar-thin">
        ...
        {design.notes
          ...
          .map((n) => {
            ...
            return (
              <li key={n.id} className="border-b border-[var(--color-line)] p-4">
                {editing === n.id ? (
                  <div>
                    <textarea rows={3} value={editText} onChange={(e) => setEditText(e.target.value)} className="block w-full rounded-sm border border-[var(--color-line)] bg-white p-2 text-sm" />
                  ...
                  <>
                    ...
                    <div className="mt-2 flex flex-wrap items-center gap-2 text-[11px] text-[var(--color-ink-soft)]">
                      {def && item ? (
                        <button type="button" onClick={() => setUi({ selectedId: item.id })} className="rounded-full border border-[var(--color-brass)] px-2 py-0.5 text-[var(--color-brass-dark)] hover:bg-[var(--color-brass)] hover:text-[var(--color-navy)]">
                          📍 {def.short}
                        </button>
                      ) : (
                        <span className="rounded-full border border-[var(--color-line)] px-2 py-0.5">General</span>
                      )}
