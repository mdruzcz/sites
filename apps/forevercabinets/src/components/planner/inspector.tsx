    ...
    return (
      <div className="flex h-full min-h-0 flex-col">
        <div className="border-b border-[var(--color-line)] p-4">
          <p className="text-[10px] uppercase tracking-[0.25em] text-[var(--color-brass-dark)]">Selected</p>
          <div className="mt-2 flex gap-3">
            <div className="relative h-20 w-20 shrink-0 overflow-hidden rounded-sm border border-[var(--color-line)] bg-white">
          ...
          {def.cabinet && (
            <Link href={`/cabinets/${def.cabinet.slug}`} target="_blank" className="mt-2 inline-block text-[12px] underline underline-offset-4 decoration-[var(--color-brass)]">
            ...
            {isCorner ? (
              <div className="mt-2 grid grid-cols-2 gap-1.5">
                {([0, 1, 2, 3] as CornerId[]).map((c) => (
                  <button
                    ...
                    className={`rounded-sm border px-2 py-1.5 text-[11px] ${selected.corner === c ? "border-[var(--color-navy)] bg-[var(--color-navy)] text-white" : "border-[var(--color-line)] bg-white hover:border-[var(--color-navy)] disabled:opacity-40"}`}
                  >
                    {CORNER_NAMES[c]}
                  </button>
                ))}
              ...
              <>
                ...
                <label className="mt-3 block text-[11px] text-[var(--color-ink-soft)]">
                  Move to
                  <select
                    className="mt-1 block h-9 w-full rounded-sm border border-[var(--color-line)] bg-white px-2 text-sm"
          ...
          {itemNotes.length > 0 && (
            <section>
              ...
              <ul className="mt-2 space-y-1.5">
                {itemNotes.map((n) => (
                  <li key={n.id} className="rounded-sm border border-[var(--color-line)] bg-[var(--color-cream-warm)] p-2 text-[12px]">
                    {n.text}
                  </li>
                ))}
              </ul>
...
function SurfaceInspector() {
  ...
  return (
    <div className="flex h-full min-h-0 flex-col">
      <div className="border-b border-[var(--color-line)] p-4">
        <p className="text-[10px] uppercase tracking-[0.25em] text-[var(--color-brass-dark)]">{isIsland ? "Island" : "Selected wall"}</p>
        <h3 className="mt-1 font-display text-xl text-[var(--color-navy)]">{describeSurface(s)}</h3>
        <p className="mt-1 text-[12px] text-[var(--color-ink-soft)]">
      ...
      <div className="min-h-0 flex-1 overflow-y-auto scrollbar-thin p-4 space-y-5">
          ...
          <div className="mt-2 grid grid-cols-2 gap-1.5">
            {([0, 1, 2, 3] as WallId[]).map((w) => (
              <button
                ...
                className={`rounded-sm border px-2 py-1.5 text-[11px] ${s === w ? "border-[var(--color-navy)] bg-[var(--color-navy)] text-white" : "border-[var(--color-line)] bg-white hover:border-[var(--color-navy)] disabled:opacity-40"}`}
              >
                {WALL_NAMES[w]}
              </button>
            ))}
            <button
              ...
              className={`col-span-2 rounded-sm border px-2 py-1.5 text-[11px] ${s === "island" ? "border-[var(--color-navy)] bg-[var(--color-navy)] text-white" : "border-[var(--color-line)] bg-white hover:border-[var(--color-navy)]"}`}
            >
            ...
            <div className="mt-2 space-y-2 text-[12px]">
              ...
              <div className="grid grid-cols-2 gap-1.5">
                {(["front wall", "left wall", "back wall", "right wall"] as const).map((label, f) => (
                  <button
                    ...
                    className={`rounded-sm border px-2 py-1.5 text-[11px] capitalize ${design.island.facing === f ? "border-[var(--color-navy)] bg-[var(--color-navy)] text-white" : "border-[var(--color-line)] bg-white hover:border-[var(--color-navy)]"}`}
                  >
                    {label}
                  </button>
                ))}
              ...
              <div className="grid grid-cols-2 gap-2">
                <label className="text-[11px] text-[var(--color-ink-soft)]">
                  ...
                  <input
                    type="number"
                    className="mt-1 h-9 w-full rounded-sm border border-[var(--color-line)] bg-white px-2 text-sm"
        ...
        {!openWall && !(isIsland && !design.island.enabled) && (
          <section>
            ...
            <div className="mt-2 space-y-1.5 text-[12px]">
              <select className="h-9 w-full rounded-sm border border-[var(--color-line)] bg-white px-2 text-sm" value={opts.mix} onChange={(e) => setOpts({ ...opts, mix: e.target.value as AutofillOptions["mix"] })}>
        ...
        {items.length > 0 && (
          <section>
            ...
            <ul className="mt-2 divide-y divide-[var(--color-line)] border border-[var(--color-line)] bg-white">
              {items
                .slice()
                .sort((a, b) => a.t - b.t)
                .map((i) => {
                  ...
                  return (
                    <li key={i.id}>
                      <button type="button" onClick={() => setUi({ selectedId: i.id })} className="flex w-full items-center justify-between px-3 py-2 text-left text-[12px] hover:bg-[var(--color-sandstone-soft)]">
                        <span>
                          <span className="font-mono text-[var(--color-brass-dark)]">{d?.short}</span> · {formatInches(d?.width ?? 0)} {d?.level === "wall" ? "wall" : ""}
                        </span>
