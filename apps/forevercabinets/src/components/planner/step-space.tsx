  ...
  return (
    <div className="mx-auto grid max-w-7xl gap-6 px-4 py-6 lg:grid-cols-[400px_minmax(0,1fr)] lg:px-6">
      <div className="space-y-7">
        <header>
          <p className="text-[11px] uppercase tracking-[0.3em] text-[var(--color-brass-dark)]">Step 1 of 3</p>
          <h1 className="mt-1 font-display text-3xl text-[var(--color-navy)]">Define your space</h1>
          ...
          <div className="mt-2 grid grid-cols-2 gap-2 sm:grid-cols-3 lg:grid-cols-2">
            {ROOM_PRESETS.map((p) => (
              <button
                ...
                className={`rounded-sm border p-2 text-left transition-colors ${activePreset?.id === p.id ? "border-[var(--color-navy)] bg-[var(--color-sandstone-soft)]" : "border-[var(--color-line)] bg-white hover:border-[var(--color-navy)]"}`}
              >
                <ShapeGlyph open={p.openWalls} />
                <p className="mt-1.5 text-[13px] font-medium leading-tight text-[var(--color-navy)]">{p.label}</p>
        ...
        <section>
          ...
          <div className="mt-2 grid grid-cols-2 gap-1.5">
            {([0, 1, 2, 3] as WallId[]).map((w) => {
              ...
              return (
                <label key={w} className={`flex cursor-pointer items-center gap-2 rounded-sm border px-3 py-2 text-[13px] ${open ? "border-[var(--color-line)] bg-white text-[var(--color-ink-soft)]" : "border-[var(--color-navy)] bg-[var(--color-sandstone-soft)]"}`}>
        ...
        <section>
          ...
          <div className="mt-2 flex flex-wrap items-end gap-2">
            <label className="text-[11px] text-[var(--color-ink-soft)]">
              Type
              <select value={newKind} onChange={(e) => setNewKind(e.target.value as "window" | "door")} className="mt-1 block h-9 rounded-sm border border-[var(--color-line)] bg-white px-2 text-sm">
            ...
            <label className="text-[11px] text-[var(--color-ink-soft)]">
              On
              <select value={newWall} onChange={(e) => setNewWall(Number(e.target.value) as WallId)} className="mt-1 block h-9 rounded-sm border border-[var(--color-line)] bg-white px-2 text-sm">
  ...
  return (
    <label className="block text-[11px] text-[var(--color-ink-soft)]">
      ...
      <input
        ...
        className="mt-1 block h-10 w-full rounded-sm border border-[var(--color-line)] bg-white px-2 text-base text-[var(--color-navy)] focus:border-[var(--color-navy)] focus:outline-none"
      />
...
function OpeningRow({ o, room, onChange, onRemove }: { o: Opening; room: { width: number; depth: number; openWalls: WallId[] }; onChange: (patch: Partial<Opening>) => void; onRemove: () => void }) {
  ...
  return (
    <li className="rounded-sm border border-[var(--color-line)] bg-white p-2.5">
      <div className="flex items-center justify-between gap-2">
        <p className="text-[13px] font-medium capitalize text-[var(--color-navy)]">
      ...
      <label className="mt-1.5 block text-[11px] text-[var(--color-ink-soft)]">
        Move to wall
        <select value={o.wall} onChange={(e) => onChange({ wall: Number(e.target.value) as WallId })} className="ml-2 h-8 rounded-sm border border-[var(--color-line)] bg-white px-1 text-xs">
          {([0, 1, 2, 3] as WallId[]).filter((w) => !room.openWalls.includes(w)).map((w) => (
            <option key={w} value={w}>
              {WALL_NAMES[w]}
            </option>
  ...
  return (
    <label className="block text-[10px] uppercase tracking-wider text-[var(--color-ink-soft)]">
      ...
      <input
        ...
        className="mt-0.5 block h-9 w-full rounded-sm border border-[var(--color-line)] bg-white px-1.5 text-sm normal-case tracking-normal text-[var(--color-navy)]"
      />
    </label>
  );
}
