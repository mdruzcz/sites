  ...
  return (
    <div className="flex h-full min-h-0 flex-col">
      <header className="flex items-center justify-between border-b border-[var(--color-line)] px-4 py-3">
        <div>
          <p className="text-[10px] uppercase tracking-[0.25em] text-[var(--color-brass-dark)]">{mode === "save" ? "Save" : "Share"}</p>
          <h3 className="font-display text-lg text-[var(--color-navy)]">{mode === "save" ? "Save your design" : "Share this design"}</h3>
        </div>
      ...
      <div className="min-h-0 flex-1 overflow-y-auto scrollbar-thin p-4 space-y-6">
        {mode === "save" && (
          <section>
            ...
            <input id="design-name" value={name} onChange={(e) => setName(e.target.value)} className="mt-1 h-10 w-full rounded-sm border border-[var(--color-line)] bg-white px-3 text-sm" />
        ...
        {mode === "share" && (
          <section>
            <p className="text-[12px] text-[var(--color-ink-soft)]">The link contains the whole design — open it on any device, or send it to us with your quote request.</p>
            {busy && !best && <p className="mt-2 text-sm">Preparing link…</p>}
            {best && (
              <>
                <textarea readOnly value={best} rows={shortLink ? 2 : 4} className="mt-2 block w-full rounded-sm border border-[var(--color-line)] bg-[var(--color-cream-warm)] p-2 font-mono text-[11px]" onFocus={(e) => e.currentTarget.select()} />
                <div className="mt-2 flex flex-wrap gap-2">
                  <button type="button" onClick={() => copy(best)} className="btn-primary text-sm">
                    Copy link
                  </button>
                  <a href={`mailto:?subject=${encodeURIComponent(`Kitchen design — ${design.name}`)}&body=${encodeURIComponent(`Here's my kitchen design from Forever Cabinets:\n\n${best}`)}`} className="btn-secondary text-sm">
                    Email it
                  </a>
