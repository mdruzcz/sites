...
function makeMats() {
  return {
    ...
    ghost: new THREE.MeshStandardMaterial({ color: "#c5a059", roughness: 0.6, transparent: true, opacity: 0.25 }),
  };
}
function mats(): Mats {
    ...
    <group position={[centre.x, 0, centre.y]} rotation={[0, yaw, 0]}>
      {!hasItems && (
        <mesh position={[0, BASE_H / 2, 0]} material={m.ghost} onPointerDown={readonly ? undefined : onPointerDown}>
          ...
          <Edges color="#c5a059" lineWidth={1} />
        </mesh>
      )}
      ...
      <mesh position={[0, 0.15, 0]} rotation={[-Math.PI / 2, 0, 0]} onPointerDown={readonly ? undefined : onPointerDown}>
        ...
        <meshBasicMaterial color={active ? "#c5a059" : "#0d1b2a"} transparent opacity={active ? 0.22 : 0.08} />
      </mesh>
      {hasItems && (
