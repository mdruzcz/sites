  ...
  return (
    <div className="print-show print-sheet">
      <div className="print-area">
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "baseline", borderBottom: "2px solid #0d1b2a", paddingBottom: 8 }}>
          <div>
            <p style={{ fontSize: 10, letterSpacing: "0.3em", textTransform: "uppercase", color: "#a8853e", margin: 0 }}>Forever Cabinets · Kitchen design sheet</p>
            <h1 style={{ fontSize: 24, margin: "4px 0 0 0" }}>{design.name}</h1>
          </div>
          <p style={{ fontSize: 11, color: "#3a4a5c", margin: 0 }}>
            forevercabinets.ca · {new Date(design.updatedAt).toLocaleDateString("en-CA")}
          </p>
        </div>
      ...
      <div className="print-area" style={{ display: "grid", gridTemplateColumns: ui.snapshot ? "1fr 1fr" : "1fr", gap: 12, marginTop: 12 }}>
        <figure style={{ margin: 0, border: "1px solid #e8e0d3" }}>
          <figcaption style={{ fontSize: 10, textTransform: "uppercase", letterSpacing: "0.2em", padding: "4px 8px", borderBottom: "1px solid #e8e0d3" }}>Floor plan</figcaption>
          <FloorView design={design} readonly className="w-full" ariaLabel="Floor plan" />
        </figure>
        {ui.snapshot && (
          <figure style={{ margin: 0, border: "1px solid #e8e0d3" }}>
            <figcaption style={{ fontSize: 10, textTransform: "uppercase", letterSpacing: "0.2em", padding: "4px 8px", borderBottom: "1px solid #e8e0d3" }}>3D view</figcaption>
      ...
      {surfaces.map((s) => (
        <figure key={String(s)} className="print-area" style={{ margin: "12px 0 0 0", border: "1px solid #e8e0d3" }}>
      ...
      <section className="print-area" style={{ marginTop: 14 }}>
        ...
        <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 11 }}>
          <thead>
            <tr style={{ background: "#f0e7dc", textAlign: "left" }}>
          ...
          <tbody>
            {review.parts.map((p) => (
              <tr key={p.sku} style={{ borderBottom: "1px solid #e8e0d3" }}>
                <td style={{ padding: "4px 6px", fontFamily: "monospace", color: "#a8853e" }}>{p.sku}</td>
            ...
            {addons.map((a) => (
              <tr key={`a-${a.sku}`} style={{ borderBottom: "1px solid #e8e0d3", color: "#3a4a5c" }}>
                <td style={{ padding: "4px 6px", fontFamily: "monospace", color: "#a8853e" }}>{a.sku}</td>
