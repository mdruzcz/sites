// Optional cloud storage for short share links (table: fc_designs — see supabase/fc_designs.sql).
...
export async function storeDesign(raw: unknown): Promise<{ code: string } | { error: string }> {
  ...
  try {
    ...
    for (let attempt = 0; attempt < 3; attempt++) {
      const code = makeCode();
      const { error } = await supabase.from("fc_designs").insert({
        code,
        name: design.name,
        design,
...
export async function fetchDesignByCode(code: string): Promise<Design | null> {
  ...
  try {
    const supabase = createServerSupabase();
    const { data, error } = await supabase.from("fc_designs").select("design").eq("code", code.toLowerCase()).maybeSingle();
