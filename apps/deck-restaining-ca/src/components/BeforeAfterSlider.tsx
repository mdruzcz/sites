...
export function BeforeAfterSlider({ beforeSrc, afterSrc, label }: BeforeAfterSliderProps) {
  ...
  return (
    <div className="space-y-2">
      ...
      >
        ...
        >
          <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-10 h-10 bg-white rounded-full shadow-lg flex items-center justify-center">
            <svg className="w-5 h-5 text-[var(--wood-dark)]" fill="none" stroke="currentColor" viewBox="0 0 24 24">
      ...
      <p className="text-sm text-stone-500 text-center font-medium">{label}</p>
    </div>
  );
}
