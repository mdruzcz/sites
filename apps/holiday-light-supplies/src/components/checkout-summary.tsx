...
export function CheckoutSummary({ cart }: { cart: Cart }) {
  ...
  const remaining = Math.max(0, 150 - cart.subtotal_cad);
  const progress = Math.min(100, (cart.subtotal_cad / 150) * 100);
    ...
    <aside className="space-y-4 lg:sticky lg:top-24 lg:self-start">
      ...
      <div className="rounded-xl border border-[var(--color-border)] bg-white p-4">
        <dl className="space-y-2 text-sm">
          ...
          <Row
            label="Shipping"
            value={cart.subtotal_cad >= 150 ? "FREE" : "Calculated at next step"}
            valueColor={cart.subtotal_cad >= 150 ? "text-[var(--color-success)]" : undefined}
          />
      ...
      <div className="rounded-xl bg-[var(--color-brand-soft)] p-4 text-xs text-[var(--color-brand)]">
        ...
        <p className="mt-2 text-[var(--color-brand-dark)]/80">
          Returns within 30 days · 5-year LED warranty · Customer service across Ontario.
        </p>
      </div>
    </aside>
  );
