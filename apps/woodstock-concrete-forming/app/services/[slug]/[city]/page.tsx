...
export async function generateMetadata({
  ...
  return {
    ...
    description: `Expert ${service.title.toLowerCase()} in ${city.name}, Ontario. Natural broom finish and coloured stamped concrete from ${site.name} — free on-site quote, written warranty.`,
    openGraph: {
      title: `${service.title} in ${city.name} | ${site.name}`,
      description: `Expert ${service.title.toLowerCase()} in ${city.name}, ON. Free quote, written warranty.`,
    },
  };
}
...
export default async function CityServicePage({
  ...
  const cityContent: Record<string, { headline: string; intro: string }> = {
    woodstock: {
      ...
      intro: `Woodstock homeowners trust us for ${service.title.toLowerCase()} that last. Oxford County's freeze-thaw winters demand proper base prep, air-entrained concrete, and reinforced steel — and that's exactly what every pour gets.`,
    },
    brantford: {
      headline: `${service.title} in Brantford, Ontario`,
      intro: `Brantford homeowners choose us for ${service.title.toLowerCase()} built to handle Brant County's harsh winters. Properly reinforced, properly drained, and backed by a written warranty.`,
    },
    cambridge: {
      headline: `${service.title} in Cambridge, Ontario`,
      intro: `Cambridge and Waterloo Region homeowners rely on us for ${service.title.toLowerCase()} engineered for Ontario weather. We serve Cambridge, Preston, and Hespeler with the same reinforced pours and written warranty we deliver across Oxford County.`,
    },
  };

  const content = cityContent[citySlug] || {
    headline: `${service.title} in ${city.name}, ON`,
    intro: `Expert ${service.title.toLowerCase()} in ${city.name}, Ontario. Reinforced, properly graded, and backed by a written workmanship warranty.`,
  };
