...
interface Props {
  ...
  intro?: string;
  crumbs?: Crumb[];
  /** Show the short quote form beside the heading. */
  form?: boolean;
  formCity?: string;
  formService?: string;
  compact?: boolean;
...
/** Photo hero with breadcrumb JSON-LD and an optional inline quote form. */
export function PageHero({ photo, photoAlt, eyebrow, title, intro, crumbs = [], form = true, formCity, formService, compact = false }: Props) {
  ...
  const jsonLd = {
    ...
    "@type": "BreadcrumbList",
    itemListElement: trail.map((c, i) => ({ "@type": "ListItem", position: i + 1, name: c.label, item: c.href ? `${site.url}${c.href === "/" ? "" : c.href}` || site.url : undefined })),
  };
  return (
    <section className="relative isolate overflow-hidden bg-[var(--ink)] text-white">
      <Script id={`crumbs-${crumbs.map((c) => c.label).join("-").toLowerCase().replace(/[^a-z]+/g, "-") || "home"}`} type="application/ld+json" dangerouslySetInnerHTML={{ __html: JSON.stringify(jsonLd) }} />
      ...
      <div className={`shell relative grid gap-10 pt-28 lg:grid-cols-[1.2fr_1fr] lg:items-center ${compact ? "pb-14 md:pt-32 md:pb-20" : "pb-16 md:pt-36 md:pb-24"}`}>
        <div className="reveal">
          ...
          <div className="mt-7 flex flex-wrap gap-3">
            <a href="#contact" className="btn-candy lg:hidden">Get a free quote</a>
