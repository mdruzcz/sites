...
export function ServicePage({ c }: { c: ServicePageContent }) {
  ...
  return (
    <>
      ...
      <NavBar />
      <PageHero photo={c.hero} eyebrow={c.eyebrow} title={c.h1} intro={c.intro} crumbs={[{ label: "Services", href: "/services" }, { label: svc?.name ?? c.eyebrow }]} formService={c.formService} />
      ...
      <FAQ faqs={c.faqs} title={`${svc?.shortName ?? c.eyebrow} questions`} />
      <Contact />
      <Footer />
      <CallNowFab />
    </>
