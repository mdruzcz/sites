...
export function NavBar() {
  ...
  return (
    <header className={`fixed inset-x-0 top-0 z-50 transition-all duration-300 ${scrolled || open ? "bg-white/95 shadow-[0_1px_0_var(--line),0_10px_30px_-18px_rgba(27,34,48,0.35)]" : "bg-white/80"}`}>
      <nav className="shell">
        <div className="flex h-[76px] items-center justify-between gap-4">
          <Link href="/" className="flex-shrink-0" aria-label="Festive Holiday Lighting home">
            <Image src="/images/logo.png" alt="Festive Holiday Lighting" width={409} height={230} priority className="h-14 w-auto md:h-16" />
          </Link>
      ...
      {mounted && open && createPortal(
        <div className="fixed inset-x-0 bottom-0 top-[76px] z-40 overflow-y-auto bg-white lg:hidden">
          <div className="shell flex flex-col py-6">
            {[...navLinks, { label: "FAQ", href: "/faq" }, { label: "Contact", href: "/contact" }].map((l) => (
