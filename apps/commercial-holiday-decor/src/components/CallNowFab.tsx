...
export function CallNowFab() {
  return (
    <div className="fixed bottom-5 right-5 z-40 md:hidden">
      <span
        aria-hidden
        className="absolute inset-0 rounded-full bg-[var(--color-green)] motion-safe:animate-[fab-ring_2.6s_ease-out_infinite]"
      />
      <a
        href={site.phoneHref}
        aria-label={`Call ${site.name} at ${site.phone}`}
        className="relative inline-flex min-h-[52px] items-center gap-2.5 rounded-full bg-[var(--color-green)] px-5 text-sm font-semibold text-white shadow-[0_10px_30px_rgba(20,67,47,0.45)] transition hover:bg-[var(--color-green-dark)]"
      >
